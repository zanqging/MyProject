...
- (void)layoutSubviews
{
    ...
    if (!self.isResizing) {
        CGRect rectCrop=self.scrollView.frame;
        rectCrop.size.height=rectCrop.size.width;  //固定比例
        rectCrop.origin.y=(self.scrollView.frame.size.height-rectCrop.size.height)/2;
        [self layoutCropRectViewWithCropRect:rectCrop];
        [self zoomToCropRect:rectCrop];
